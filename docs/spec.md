...
- link: 要件定義書|https://example.com/doc
- link: Slackチャンネル|https://example.com/slack

案件の概要・背景をここに自由記述（複数行可）。
...
- `{#id}` で各要素の一意IDを保持する（`prj-` / `ph-` / `t-` 接頭辞）。システムが生成・管理する内部の参照キーで、`hours.csv` の `projectId`・`hours-mapping.json` のルールから参照される。**ポータルUI上では編集できない**（安定性を保つため）。
- `code` は社内でプロジェクトを管理するための表示用パラメータ。内部ID（`{#id}`）とは完全に独立しており、他ファイルからは参照されない。ポータルUI上で自由に編集できる。
- `link` はラベルとURLを `|` 区切りで保持する（`- link: <ラベル>|<URL>`）。1行につき1件、複数行で複数登録できる。
- メタデータ（`code`/`client`/`status`/.../`link`）の後、次の見出し（`##`）までの自由記述行が概要（`overview`）。複数行可。
...
- 案件情報カード: 案件名・クライアント名・プロジェクトコード（社内管理用の表示パラメータ）・概要・関連リンク（ラベル＋URL、複数登録可）をポータルUI上で編集できる。プロジェクトコードは `hours.csv`/`hours-mapping.json` が参照する内部ID（`{#id}`、システムが生成・管理し編集不可）とは別のフィールドで、他ファイルへの参照関係を持たない単なる表示用ラベル。
- 変更は `projects.md` に保存。
...
4. 案件の割当・修正は画面上でクライアント→案件の2段階プルダウンで行う（自動割当済みのイベントも含め、全イベントで随時修正可能。任意でルール追加）。
...
- 割当UIはプレビューテーブルに「クライアント」「案件」の2列を持つ。クライアント列で `projects.md` の `client` を選択すると、案件列の選択肢がそのクライアント配下の案件に絞り込まれる。案件列で選択して初めて割当が確定する。
- クライアント列の初期値は、既に割当済みの案件があればその `client`。未割当の場合、件名が `[クライアント名]` のように `[]` で囲まれ、かつそのテキストが既存クライアント名と一致すればそれを初期値とする（`hours-mapping.json` のルールにヒットしなかった場合の初期値ヒントに過ぎず、案件列で選ぶまでは未割当のまま）。
- プレビューテーブルの「対象外」列にチェックを入れると、そのイベントは集計・確定処理から除外される（案件が未割当のままでも「確定へ進む」のブロック対象にならず、案件が割当済みでも工数の集計に含まれない）。私的な予定など、月次の工数として扱いたくない予定を除外する用途。
